...
option_list <- list(
  make_option("--process", type = "character", default = "pqtl",
              help = "Choose process: 'eqtl' or 'pqtl'"),
...
if (opt$process == "eqtl") {
  ...
  eQTLs <- eQTLs %>%
    ...
    mutate(CHR = gsub("chr", "", `#CHROM`), #create ID column
           ALT = A1) #create ALT column
  eQTLs <- eQTLs %>%
    mutate(chr_pos = (CHR %&% ":" %&% POS))
  ...
  for (target_gene in genes$genes) {
    gene_eQTL <- eQTLs %>% filter(grepl(paste0("^", target_gene), gene_id))
    
    joined = inner_join(gene_eQTL,gwas, by=c("chr_pos"="chr_pos"))
  ...
  for (target_gene in genes$genes) {
    ...
    joined = inner_join(pQTLs,gwas, by=c("chr_pos"="chr_pos"))
