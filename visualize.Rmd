...
if(gwas.count == 0 | twas.count == 0){
  print('As SUSIE found 0 causal variants in the GWAS data and/or the TWAS data, we conclude that the two traits are not colocalized at a given SNP in this region.')
}
```
...
if(gwas.count != 0 & twas.count != 0){
  print(knitr::kable(susie.sum, caption = 'coloc Summary Results', ))
  ...
  for(i in sigrows){
    sensitivity(susie.res,"H4 > 0.5",row=i,dataset1=gwassusiecoloc,dataset2=twassusiecoloc)
      ...
      print('No further analysis for this region and phenotype neccessary!')
}